        ...
        for μ in μ_array:
            ...
            for snr in snr_array:
                for p in p_array:
                    ...
                    β = bo.scale_norm(np.ones(p), snr)
                    for n in n_array:
                        X_train = np.ascontiguousarray(X[:n, :p])
                        Y_train = bo.compute_Y(X_train, β, ε)
                        if len(τ_grid) != 0:
                        ...
                        for τ in τ_array:
                            ...
                            test_MSE = bo.calculate_MSE(β_hat, X_test, β=β)
                            result_arr[idx] = np.array([*params, test_MSE])
                            idx += 1
                            progress.update(1)
    return result_arr
